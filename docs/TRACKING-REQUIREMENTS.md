...
| GA4 | на `lp` нет; у www.gambarian.com есть `G-MNNYPHLP7Z` (зашит в страницу, без GTM). Доступа у ADFIX нет: ни один из трёх GA4-коннекторов ресурс не видит, приглашений в почте нет (2026-09-22) | `clients/gambarian/context.md` (ADFIX OS); MCP GA4; почта |
| GTM | контейнера для `lp` нет; приглашений в почте нет | почта (API GTM не проверен — MCP не подключался) |
...
| 2б | GA4 и GTM для `lp`: создать под `gamba.office@gmail.com` и выдать ADFIX доступ, или создаю я под ADFIX и передаю | создать под `gamba.office@gmail.com`: ресурсы остаются у клиента | ждёт |
