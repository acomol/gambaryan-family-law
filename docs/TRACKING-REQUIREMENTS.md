...
| 5 | WhatsApp — отдельная цель «Обращение» в Ads | да, отдельной целью; лидом не считать (клик ≠ сообщение) | решено 24.09 (архитектор): в Ads вспомогательная; применение ждёт «да» владельца |
| 6 | Конверсии «звонок из объявления» (номер переадресации Google, звонок ≥ 60 с) | включать осознанно: такую конверсию Google потом нельзя сделать вторичной (E13) | решено 24.09 (архитектор): основная — звонок ≥ 60 с из объявления = обращение к адвокату; в аккаунте уже основная, правка не нужна |
...
| «×» / «Продолжить на сайте» на карточке успеха | сворачивает карточку в строку-подтверждение | не отслеживаем: без сети и без перезагрузки | — | есть |
| «Изменить контакты» на свёрнутой строке | тот же путь, что «Указать другие контакты» | `lead_corrected` {submission_id, corrects_submission_id} — **не** `generate_lead` | GA4 | есть |
