...
**Инфраструктура АКТИВИРОВАНА и живёт в проде** — параграф ниже («код и
тесты готовы, инфраструктура НЕ активирована») устарел, оставлен как есть
только внутри §1–§16 для истории ревью; текущее состояние заменяет его.
Источник каждой строки — живые readback'и координатора, `docs/STATE-2026-09-24.md`.

- **Cloudflare Pages** (`lp.gambarian.com`, аккаунт `4799e9f7…`): `LEADS_KV`,
  `LEADS_DB` (D1), `LEADS_ARCHIVE` (R2) привязаны И на Production, И на
  Preview (два независимых набора id — см. таблицу в §3). `ADMIN_PASSWORD` и
  `ALBATO_WEBHOOK_URL` — секреты, только Production; `ADMIN_USER` — обычная
  переменная (не секрет), тоже только Production.
  Миграции D1 (`attempt_count`, `next_attempt_at`, таблица `cron_health`)
  применены на prod и preview 2026-09-23.
- **Cron-worker `gambarian-lead-cron`** — живой, оба триггера включены:
  `*/5 * * * *` (свип с экспоненциальным backoff 5/10/20/40 мин, потолок
  1 час; один `KV.put` на кандидата за прогон, пропуск без
  `ALBATO_WEBHOOK_URL`) и `0 * * * *` (часовой D1→R2 дамп; UTC-час 02
  добавляет verify/purge/недельный heartbeat; UTC-час 18 добавляет
  `reconcile` + janitor удалённых лидов, §10). **История расписания**
  (координатор, `docs/STATE-2026-09-24.md`, заменяет разрозненные заметки
  в §13/§14 ниже): 23.09 ~18:1x UTC — временная пара `*/5` + `30 2`;
  ~19:5x UTC — только `30 2` (защита KV-квоты, деплоем, не ручным действием
  владельца); round-6 деплой (merge `6a8eac2`) — вернул `*/5` + `0 *`;
  round 7 и v2-деплои (`d20b9db`, затем `71fe20bc`, `1e1523a0`) — без
  изменений, `*/5` + `0 *`.
- **`GET /health` подтверждён живым readback'ом** (2026-09-24 ~08:45 IDT):
  `backup.last_ok_at=2026-09-24T05:00:13Z`, `integrity_ok=true`, sweep
  `ok` в 05:30Z, `stuck_leads=0`, `albato_configured=true`. Контракт —
  §12, без изменений.
- **Офисное письмо v2** (§15–§16) — **задеплоено и живёт через Albato**,
  вопреки завершающей строке §16 «не задеплоено» (устарела, оставлена для
  истории ревью). Контрольная заявка `b4f5e872` (24.09, 08:30 IDT) дошла до
  всех 5 адресов офиса за 2 секунды. Финальное приёмочное «ок» владельца на
  вид письма в Gmail на телефоне — **не проверено этой сессией** (открытый
  пункт, см. `docs/STATE-2026-09-24.md` «Open items»).
- **Workers Free-план — лимит триггеров исчерпан на весь аккаунт.**
  5 cron-триггеров всего (не на воркер — на аккаунт), все заняты: 3 у
  `assuta-lead-cron` + 2 у `gambarian-lead-cron`. Добавить третий триггер
  Гамбаряну без удаления чужого — нельзя. KV: 1000 write/сутки на аккаунт,
  общий с Assuta. Cron-инвокация: 10 мс CPU, 50 D1-запросов за вызов.
  Источники (прочитано 2026-09-23): developers.cloudflare.com/workers/platform/limits,
  /workers/platform/pricing, /kv/platform/limits, /d1/platform/limits.
- **D1 Time Travel** — восстановление D1 на любую минуту за последние 7
  дней, включено всегда, без доплаты:
  `wrangler d1 time-travel restore gambarian-leads --timestamp=<unix>`.
  Источник: developers.cloudflare.com/d1/reference/time-travel (прочитано
  2026-09-23). Разбор шагов — `docs/RUNBOOK-LEADS.md`.

**Статус ниже (первый абзац раздела) — исторический,** описывал состояние
ДО активации; сохранён без изменений, потому что на него ссылаются §1/§3
как на объяснение graceful-degradation ветки, которая по-прежнему в коде
(просто больше не в этом состоянии на проде).
...
**На 24.09 прод живёт в правом столбце** («Состояние с биндингами») —
таблица описывает обе ветки кода (биндинги есть/нет), обе по-прежнему
существуют в `hasDurableStorage(env)`, но live-состояние сейчас — правое.
Крон-воркер и админка ниже — уже задеплоены и активны (см. «Состояние на
24.09» выше), таблица оставлена как есть для описания самого механизма
...
созданы вне этого изменения репозитория). **Обновление 24.09: с тех пор
всё привязано и задеплоено** — см. «Состояние на 24.09» в начале файла;
шаги 1–4 ниже описывают, КАК это было сделано (историческая инструкция по
активации), не текущий пробел. `cron-worker/wrangler.toml` содержит реальные
PRODUCTION id (не плейсхолдеры) — см. таблицу ниже; Pages-биндинги
настраиваются отдельно, в дашборде проекта, не в файле этого репозитория.
...
**Обновление 24.09: эти id ПРОВЕРЕНЫ живым Cloudflare API** (координатор,
`docs/STATE-2026-09-24.md`) — заменяет предупреждение «не проверены
этой сессией», которое было верным на момент написания шагов активации
ниже. Актуальный список биндингов подтверждён напрямую, доп. `wrangler kv
namespace list`/`d1 list`/`r2 bucket list` не требуется для их проверки
(но полезен, если состояние снова расходится).
...
1. **Cloudflare Pages** (`functions/api/lead.js`, `admin.js`, `lead-dead-letter.js`) —
   **у Pages ДВА независимых набора биндингов**, Production и Preview
   ...
   - секреты (Cloudflare Pages, не в Git; **свои значения для Production и Preview**):
     ...
     ниже); опционально `ADMIN_USER` — это **обычная переменная, не секрет**
     (секрет только `ADMIN_PASSWORD`). **`ADMIN_ALLOWED_EMAILS` не используется**
...
**Контекст:** прод работает на `6034203`. Уточнение истории cron (координатор,
`docs/STATE-2026-09-24.md`, заменяет прежнюю формулировку): на этот момент (до
round-6 деплоя) в Cloudflare реально настроен только `"30 2 * * *"` — `*/5`
был снят деплоем ~19:5x UTC 23.09 для защиты KV-квоты, не отключён владельцем
вручную; round-6 деплой (merge `6a8eac2`) вернул `*/5` + `0 *`.
...
**Контекст:** прод работает на `0aaf9e1` (round-6 фикс уже задеплоен). По
истории cron (координатор, `docs/STATE-2026-09-24.md`) на этот момент `*/5`
уже снова активен вместе с `0 * * * *` (вернулся ещё round-6 деплоем) — P1
ниже как раз про KV-бюджет ПРИ работающем `*/5`, а не про его отключение
(прежняя формулировка «*/5 отключён владельцем» заменена этой историей).
...
**Устарело (было на момент правки, `0fcf61b`): «не задеплоено и Albato не
тронут».** Заменено: деплой и настройка сценария в Albato выполнены через
Codex (координатор) 2026-09-24, не владельцем — письмо v1 работало в проде,
затем заменено v2 (§16). Текущий статус — «Состояние на 24.09» в начале
файла.

## 16. Редизайн офисного письма v2 (owner request, база `ad02143`)

**Контекст:** v1 (раздел 15) заработал в проде через Albato с 02:12 —
владелец открыл письмо в Gmail на телефоне, вероятно в тёмной теме, и
попросил именно перестройку структуры, не правку отступов.

**Новый порядок:** компактный хедер → заголовок+время(+тестовый бейдж) →
ОДИН контактный блок (подпись НАД значением, без двухколоночной таблицы) →
два широких кнопки-действия (отдельный блок, полная ширина карточки, НЕ
внутри ячейки телефона) → приглушённый служебный блок (источник/страница)
→ нейтральная (не бордовая) ссылка на таблицу → короткий футер.

**Публичный API не менялся** — `renderNewLeadEmail`, формат `subject`,
`escapeHtml`/`stripSubjectControlChars`/`formatJerusalemDateTime`/
`telHrefFromPhone`/`normalizeWhatsAppNumber` побайтово те же функции, что
и в v1; изменилась только HTML/CSS-композиция внутри `renderNewLeadEmail`
и приватные HTML-хелперы (`emailLabelledRowHtml`/`emailButtonHtml`/
`emailContactButtonsHtml` из v1 заменены на `stackedFieldHtml`/
`actionButtonHtml`/`actionsBlockHtml`).

**Тестовый бейдж:** `utm_source`, содержащий подстроку «test» (например
`adfix_test`, регистронезависимо) — бейдж «Тестовая заявка — не
обрабатывать» рядом с заголовком.

**Кнопки-действия:** padding-based, НЕ fix line-height — под 200% размера
текста подпись переносится и блок растёт (у него `min-height`, не
`height`), а не обрезается. Без картинок-иконок вообще (даже unicode-глиф
не поставлен — рендер в реальном Gmail нельзя было проверить из этой
среды, а бриф прямо разрешает text-only как fallback).

**Страница в служебном блоке:** `landing_path === "/"` → текст «Главная»;
иначе — сырой путь. Ссылка в обоих случаях — `https://lp.gambarian.com` +
`landing_path`.

**Contrast table** (WCAG 2.1 relative-luminance, посчитано скриптом, не на
глаз; «simulated Gmail dark-invert» — CSS `invert(1) hue-rotate(180deg)` на
весь документ, приближение, которое сам бриф задал как способ проверить
переживает ли дизайн авто-инверсию Gmail для писем без поддержки тёмной
темы; `<meta name="color-scheme" content="light">` +
`<meta name="supported-color-schemes" content="light">` должны такую
инверсию вообще предотвратить — расчёт ниже это подстраховка на случай,
если клиент их проигнорирует):

| Пара (текст/фон или элемент) | Normal | Simulated dark-invert | Порог | OK? |
|---|---|---|---|---|
| `ink` (#14191f) текст на `cardBg` (#ffffff) — имя/телефон/заголовок | 17.67:1 | 16.89:1 | 4.5:1 | ✅ |
| `ink` текст на `gold` (#f0ae1f) — «Позвонить» | 9.07:1 | 5.54:1 | 4.5:1 | ✅ |
| `ink` текст/бордер на `cardBg` — WhatsApp/«Открыть таблицу заявок» (outline) | 17.67:1 | 16.89:1 | 4.5:1 | ✅ |
| `muted` (#5b6169) на `cardBg` — подписи полей, время, служебный блок | 6.25:1 | 7.87:1 | 4.5:1 | ✅ |
| `gold` на `headerBg` (#0a0b0d) — «Адвокаты» | 10.11:1 | 6.25:1 | 4.5:1 | ✅ |
| `#ffffff` на `headerBg` — «Гамбарян & Партнёры» | 19.69:1 | 19.08:1 | 3:1 (крупный/жирный) | ✅ |
| `badgeText` (#7a4a06) на `badgeBg` (#fef3c7) — тестовый бейдж | 6.71:1 | 8.99:1 | 4.5:1 | ✅ |
| `badgeBorder` (#92400e) на `badgeBg` — граница бейджа (UI) | 6.37:1 | 9.38:1 | 3:1 | ✅ |
| `gold` заливка на `cardBg` — граница кнопки как UI-объект | 1.95:1 | 3.05:1 | 3:1 | ⚠️→✅* |
| `goldBtnBorder` (`ink`) на `gold` / на `cardBg` — обводка «Позвонить» | 9.07:1 / 17.67:1 | 5.54:1 / 16.89:1 | 3:1 | ✅ |

\* Заливка gold сама по себе даёт только 1.95:1 против белой карточки в
normal-режиме — ниже порога 3:1 для UI-компонента. Компенсировано: у кнопки
есть собственная сплошная обводка цветом `ink` (`goldBtnBorder`), которая
сама по себе даёт ≥3:1 и против заливки, и против карточки — стандартный
приём для заливок с недостаточным контрастом к фону страницы (WCAG 1.4.11
через альтернативную видимую границу).

**Скриншоты** (Playwright, `page.setContent()` — без dev-сервера, письмо
самодостаточно), тестовые данные: имя «Александра Никифорова-Штейнберг»,
телефон `+972 50 123 4567`, email, `utm_source=adfix_test` (бейдж виден на
всех кадрах) — сохранены в `I:/Temp/claude/lead-email-v2/`:
- `320-light.png`, `390-light.png` — светлая тема;
- `390-dark-invert.png` — симуляция авто-тёмной темы Gmail (`invert(1)
  hue-rotate(180deg)` на `<html>`);
- `390-text-200.png` — имя/телефон/подписи кнопок при удвоенном
  `font-size` (симуляция «крупный текст» — НЕ page zoom, чтобы не
  замаскировать баг: у кнопки только `font-size` вырос, `padding`/
  `min-height` остались авторскими — если бы был жёсткий `line-height`,
  тут было бы видно обрезание; в реальности не обрезается).
- горизонтальный скролл при 320px — проверено программно
  (`document.documentElement.scrollWidth <= clientWidth`) на каждом кадре:
  отсутствует.

**Тесты** (`test/lead-qa.mjs` T29–T31, red на `ad02143` → green здесь):
тестовый бейдж вкл/выкл по `utm_source`; «Главная» для `landing_path==="/"`
и сырой путь иначе, оба со ссылкой на `lp.gambarian.com`; regression-guard
на отсутствие `width="92"` (маркер старой двухколоночной вёрстки v1) —
доказывает, что структура правда заменена, а не просто перекрашена.
Существующие T27–T28 (escaping, WhatsApp-нормализация, apostrophe-guard) и
cron-retry блок в `cron-backup-qa.mjs` перепрогнаны без изменений — все
зелёные на новой вёрстке, значит эскейпинг/WhatsApp-правила не затронуты.

**Устарело (было на момент правки, `ad02143`): «не задеплоено и Albato не
тронут».** Заменено: v2 задеплоена и работает в проде через Albato —
контрольная заявка `b4f5e872` (24.09, 08:30 IDT) подтвердила доставку всем
5 адресатам за 2 секунды. Финальное приёмочное «ок» владельца по виду в
Gmail на телефоне — не проверено этой сессией. Текущий статус — «Состояние
на 24.09» в начале файла.
