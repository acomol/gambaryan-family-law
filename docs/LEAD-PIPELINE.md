...
**Не входило в это ревью (сознательно, не запрашивалось):** редундантный tombstone внутри самой D1-строки (например, `status='deleted'` вместо `DELETE`) — обсуждалось как альтернатива для находки 5, но потребовало бы менять семантику существующих тестов и `admin.js`-фильтров; оставлен единственный источник tombstone — KV-ключ `tomb:<id>`, проверяемый обоими свипами. Если `LEADS_KV` полностью недоступен и на момент удаления, и на момент свипа — это тоже не защищено (тот же класс остаточного риска, что и в находке 6).
