...
- **(c) Гарантия документирована здесь:** удаление гарантированно
  завершается **в течение одного интервала `reconcile` (раз в сутки,
  `0 18 * * *`)**, а не мгновенно. Для остаточного окна между re-check'ом
  (a) и возвратом ответа — теоретически возможна, но не доказано что
  ненулевая без Durable Object; janitor (b) закрывает её на следующем
  цикле в любом случае.
...
`last_ok_at` прежним, но продвигает `last_run_at` и переключает
`integrity_ok` в `false`.
