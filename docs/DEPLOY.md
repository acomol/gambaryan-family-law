...
## Preview: все двенадцать одной командой
...
Он читает байты, которые реально отдаёт Cloudflare, по каждому из 12
...
| | Что делать | Цена |
|---|---|---|
| **A. Токен** | Pages:Edit на аккаунт лендингов, Client IP пустой, в GitHub Secrets | все 12 адресов сохраняются; один разовый шаг владельца |
| **B. Новый git-проект** | создать новый Pages-проект с подключением к `gambaryan-family-law` | деплой навсегда автоматический и без ключей, но адреса меняются; 12 вариантов удобнее отдавать подпутями одного адреса, а не поддоменами |
...
**Состояние на 2026-08-17 (обновлено).** PR влит, `deploy-previews.yml` лежит
на `main`, кнопка Run workflow доступна и использована. Оба секрета —
`CLOUDFLARE_API_TOKEN` и `CLOUDFLARE_ACCOUNT_ID` — заведены как **Repository
secrets**. Это подтвердило раздел выше буквально: три прогона подряд упали с
generic `Authentication error` (код 10000) на шаге `Check token scope`, при
том что тот же токен прямым запросом с машины владельца отдаёт HTTP 200 и
видит `gambarian-landing`. Причина — то самое **Client IP Address Filtering**,
описанное выше: раннер GitHub не входит в allowlist токена.

🔴 Первые два падения были из-за собственных ошибок оператора при заводе
секрета (см. ниже), не из-за IP — важно не путать симптомы:
- прогоны 1-5: `CLOUDFLARE_API_TOKEN` не был задан вообще (только
  `ACCOUNT_ID`) — лог показывал `***` для пустой переменной, что неотличимо
  от заданной по одной лишь маске;
- следующий прогон: секрет задан через PowerShell-конвейер
  (`... | gh secret set NAME`), который дописал завершающий перевод строки к
  значению — токен, отдававший 200 в прямом запросе, ломался внутри секрета.
  Починка — `gh secret set NAME --body $var` (переменная, не конвейер).
- только ПОСЛЕ обеих починок стала видна настоящая, третья причина — IP-lock,
  который этот документ предсказывал заранее (проверено 2026-08-04).

**✅ Закрыто 2026-08-17 (вариант A).** Владелец снял Client IP Address
Filtering с личного deploy-токена в Dashboard. Понадобилась ещё вторая,
независимая починка — без неё CI продолжал бы падать под новым предлогом:

🔴 **Гейт `Check token scope` сам был сломан.** Cloudflare отдаёт успешные
ответы с отступами (`"success": true` — с пробелом), а ошибки — компактно
(`"success":false`). Проверка `grep '"success":true'` не находила успех
НИКОГДА и объявляла отказом любой нормальный ответ. После снятия IP-фильтра
это дало четвёртый по счёту ложный диагноз — «право Pages:Edit на аккаунт
не выдано», хотя запрос вернул HTTP 200 и все 9 проектов аккаунта, включая
`gambarian-landing`. Починено коммитом `7e83f9f`: три проверки переведены с
поиска подстроки на разбор JSON через `python3`.

**Подтверждено прогоном `32038757556` (2026-08-17):** все шаги success,
`Live readback` — `PASS final-dev3` + `PASS production (не должен
измениться)`. Независимая байтовая проверка совпала с CI-логом:
`final-dev3` 58 592 байта, боевой `gambarian-landing.pages.dev` 52 872 байта
с контрольным SHA-256, начинающимся `656CBCD0…`.

**Урок дороже обоих фиксов:** гейт опирался на формат сериализации чужого
API, который никто не гарантировал, и это давало ложные причины поверх
настоящей четыре раза подряд («не тот аккаунт» → «нет Account Settings:
Read» → «нет Pages:Edit»). Проверка, читающая сырой текст ответа вместо
разбора структуры, — источник шума, не диагностики. Правило и техническая
деталь занесены в общую (не только для этого проекта) память —
`rules/feedback-cf-deploy-pitfalls` в personal-brain владельца.
