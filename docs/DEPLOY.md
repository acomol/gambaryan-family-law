...
**Открыто, решение владельца (вариант A или B из таблицы двумя абзацами
выше):**
- **A (быстрее):** снять Client IP Address Filtering с личного deploy-токена
  в Dashboard → API Tokens → Edit — но это тот же токен, которым владелец
  пользуется для ручных `deploy-pages.ps1`;
- **B (чище):** выпустить отдельный CI-only токен (тот же скоуп Pages:Edit,
  тот же аккаунт лендингов, поле Client IP пустое с самого начала) и
  заменить им `CLOUDFLARE_API_TOKEN` в Secrets, не трогая личный.

Выпуск/правка Cloudflare-токена — действие владельца, агент токены не
создаёт.
