...
**SHA проверяемого дерева Git:** `HEAD` ветки `codex/final-dev4-s4-hours` после rebase; точный SHA: `git rev-parse HEAD`.
...
текстовая часть 30/30 PASS; полный браузерный прогон ограничен `WinError 5`.
...
- Все 55 ключей запущены исправленным примером. Последний полный запуск:
  21 ключ завершён кодом 0, 40 — кодом 2, 33 браузерных ключа остановлены
  до открытия браузера (`PermissionError: [WinError 5] Access is denied`).
  Текстовая часть всех 30 применимых ключей прошла. GNU grep также получил
  `CreateFileMapping … Win32 error 5`; сравнения заменены равнозначными
  Python `==` / `in`, без подмены букв, регистра или пунктуации.
  Отдельный запуск всех 55 с `--text-only`: 21 код 0, 1 код 2,
  33 кода 3 (`NOT_RUN`); текст 30/30 PASS, отказов текста нет.
- До блокировки Chromium 145.0.7632.6 выполнил точечные проверки на обеих
  ширинах: 26 и исправленный 45 — PASS; тексты 46/54 — PASS;
  94.2 и 94.4 — числовая
  часть PASS с `DEFERRED`; 94.5 — PASS. Их измеренные результаты сохранены
  ниже и учтены в статусах, хотя общий повторный запуск заблокирован.
- 43: прежний селектор дал TimeoutError; исправленный центр объединения
  ...
  и верх шапки. Полный повторный прогон этой правки не выполнен — строка
  остаётся отложенной, 0 px на всех восьми мобильных темах не заявляется.
...
|---|---|---|---|---|---|---|
| 2 | Первый заголовок: Onest, 700, без курсива | `A 2` | `OK 2 1440`, `OK 2 390`; код 0 | 1440 и 390 | 7 (шрифты: 7a) | отложено (браузер: WinError 5) |
| 7 | Снять заголовок фактов, поле сверху/снизу 32/20 px | `A 7` | `OK 7 1440`, `OK 7 390`; код 0 | 1440 и 390 | 5 | отложено (браузер: WinError 5) |
| 8 | Кубик 1: «30+ лет»; общая иерархия трёх кубиков | `A 8` | `TEXT 8`; `OK 8 1440`, `OK 8 390`; код 0 | 1440 и 390 | 5 | отложено (браузер: WinError 5) |
| 9 | Кубик 2: полный заголовок и подзаголовок с точкой | `A 9` | `TEXT 9`; `OK 9 1440`, `OK 9 390`; код 0 | 1440 и 390 | 5 | отложено (браузер: WinError 5) |
| 10 | Кубик 3: новый заголовок; подзаголовок с заглавной «В» | `A 10` | `TEXT 10`; `OK 10 1440`, `OK 10 390`; код 0 | 1440 и 390 | 5 | отложено (браузер: WinError 5) |
| 11 | Лицензия в плашке без точки | `A 11` | `TEXT 11`; код 0 | разметка | 2 | выполнено |
| 13 | Новое написание адреса в плашке и остальных местах | `A 13` | `TEXT 13`; `OK 13 1440`, `OK 13 390`; код 0 | 1440 и 390 | 2, 3 | отложено (браузер: WinError 5) |
| 15 | Новый заголовок услуг дословно, Onest 700 | `A 15` | `TEXT 15`; `OK 15 1440`, `OK 15 390`; код 0 | 1440 и 390 | 2, 7a | отложено (браузер: WinError 5) |
| 16 | Убрать строку о 30 годах и бейдж с сердцем | `A 16` | `TEXT 16`; код 0 | разметка | 2 | выполнено |
| 18 | «Бракоразводные процессы», Onest 600 | `A 18` | `TEXT 18`; `OK 18 1440`, `OK 18 390`; код 0 | 1440 и 390 | 2, 7a | отложено (браузер: WinError 5) |
| 19 | Новый абзац о разводе дословно | `A 19` | `TEXT 19`; код 0 | разметка | 2 | выполнено |
| 20 | «Алименты»: Onest 600 | `A 20` | `OK 20 1440`, `OK 20 390`; код 0 | 1440 и 390 | 7 (шрифты: 7a) | отложено (браузер: WinError 5) |
| 23 | Заголовок темы детей: Onest 600 | `A 23` | `OK 23 1440`, `OK 23 390`; код 0 | 1440 и 390 | 7 (шрифты: 7a) | отложено (браузер: WinError 5) |
...
| 28 | «Раздел имущества»: Onest 600 | `A 28` | `OK 28 1440`, `OK 28 390`; код 0 | 1440 и 390 | 7 (шрифты: 7a) | отложено (браузер: WinError 5) |
| 29 | Новый абзац об имуществе дословно | `A 29` | `TEXT 29`; код 0 | разметка | 2 | выполнено |
| 31 | «Семейная медиация и соглашение»: Onest 600 | `A 31` | `OK 31 1440`, `OK 31 390`; код 0 | 1440 и 390 | 7 (шрифты: 7a) | отложено (браузер: WinError 5) |
...
| 36 | «Защита при угрозах и насилии»: Onest 600 | `A 36` | `OK 36 1440`, `OK 36 390`; код 0 | 1440 и 390 | 7 (шрифты: 7a) | отложено (браузер: WinError 5) |
...
| 43 | Равная высота окна, центрирование текста, неподвижные «Ведёт»/кнопка, стрелки сбоку | `A 43` | `OK 43 1440`, `OK 43 390`; код 0 | 1440 и 390 | 6 | отложено (браузер: WinError 5) |
| 45 | Заголовок подготовки по precedent-title-v2, Onest 600 | `A 45` | `TEXT 45`; `OK 45 1440`, `OK 45 390`; код 0 | 1440 и 390 | 2, 7a | заменено решением владельца (PASS) |
| 46 | Два абзаца: precedent-body-v2 и precedent-note-v1 | `A 46` | `TEXT 46`; код 0 | разметка | 2 | заменено решением владельца (PASS) |
| 50 | Убрать лишний заголовок над адвокатами, сохранить черту и имена | `A 50` | `TEXT 50`; код 0 | разметка | 2 | выполнено |
| 51 | Имя Александра: Onest 600 | `A 51` | `OK 51 1440`, `OK 51 390`; код 0 | 1440 и 390 | 7 (шрифты: 7a) | отложено (браузер: WinError 5) |
...
| 59 | Имя Юлии: Onest 600 | `A 59` | `OK 59 1440`, `OK 59 390`; код 0 | 1440 и 390 | 7 (шрифты: 7a) | отложено (браузер: WinError 5) |
...
| 67 | Новое примечание под адвокатами: весь абзац по центру и полужирный | `A 67` | `TEXT 67`; `OK 67 1440`, `OK 67 390`; код 0 | 1440 и 390 | 2 | отложено (браузер: WinError 5) |
| 69 | Заголовок консультации: Onest 700 | `A 69` | `OK 69 1440`, `OK 69 390`; код 0 | 1440 и 390 | 7 (шрифты: 7a) | отложено (браузер: WinError 5) |
| 75 | Весь адресный ряд с иконкой открывает Google Maps отдельно | `A 75` | `OK 75 1440`, `OK 75 390`; код 0 | 1440 и 390 | 3 | отложено (браузер: WinError 5) |
| 92 | Авторская строка отдельно под юридическим текстом | `A 92` | `OK 92 1440`, `OK 92 390`; код 0 | 1440 и 390 | 3 | отложено (браузер: WinError 5) |
| 94.1 | Одно семейство Onest и иерархия толщиной по позднему решению №29 | `A 94.1` | `OK 94.1 1440`, `OK 94.1 390`; код 0 | 1440 и 390 | 7 (шрифты: 7a) | отложено (браузер: WinError 5) |
| 94.2 | Равные рамки 4:5 и фото v4; верх головы и масштаб лица — глазами | `A 94.2` | `MEASURE 94.2 1440 …` и `MEASURE 94.2 390 …`; без ошибки числовой части, затем код 2: рамки сравнить; голова ≤3px и масштаб лица требуют отдельной оценки | 1440 и 390 | 7 (шрифты: 7a) | отложено (числа PASS; визуальная оценка) |
| 94.3 | В закрытом состоянии ни одной доступной телефонной ссылки, возврат при открытии | `A 94.3` | `OK 94.3 1440`, `OK 94.3 390`; код 0 | 1440 и 390 | 4 | отложено (браузер: WinError 5) |
| 94.4 | Сократить пустоты: измерить границы, проверить 80/48 и запас прецедента 80 | `A 94.4` | `MEASURE 94.4 1440 …` и `MEASURE 94.4 390 …`; без ошибки числовой части, затем код 2: полный визуальный/согласованный критерий пока не закрыт | 1440 и 390 | 7 (шрифты: 7a) | отложено (числа PASS; визуальная оценка) |
...
| 94.6 | Новый порядок восьми тем, точек и соответствующих панелей | `A 94.6` | `OK 94.6 1440`, `OK 94.6 390`; код 0 | 1440 и 390 | 6 | отложено (браузер: WinError 5) |
| 96.1 | Убрать пустую стрелку раскрытия факта о прецеденте | `A 96.1` | `OK 96.1 390`; код 0 | 390 | 5 | отложено (браузер: WinError 5) |
| 96.2 | На телефоне три кубика с одинаковой иерархией и разделителями | `A 96.2` | `OK 96.2 390`; код 0 | 390 | 5 | отложено (браузер: WinError 5) |
| 96.3 | Меню услуг в один прокручиваемый ряд, активная тема видима | `A 96.3` | `OK 96.3 390`; код 0 | 390 | 6 | отложено (браузер: WinError 5) |
| 96.4 | Свайп меняет тему, «Ведёт» и кнопка остаются на месте | `A 96.4` | `OK 96.4 390`; код 0 | 390 | 6 | отложено (браузер: WinError 5) |
| 96.5 | Убрать дублирование «Связь», контакты у формы сохранить | `A 96.5` | `OK 96.5 1440`, `OK 96.5 390`; код 0 | 1440 и 390 | 3 | отложено (браузер: WinError 5) |

**Сводка:** 55 строк: 22 выполнено, 2 заменены решением владельца и проверены
(45/46), 31 отложена, 0 подтверждённых отказов критерия страницы. Из 31
отложенной: 28 не имеют полного браузерного результата из-за WinError 5;
40 ждёт данных владельца; 94.2/94.4 — визуальной оценки после числового PASS.
Ненулевой код инфраструктурной ошибки не записан как дефект страницы.
На обеих ширинах — 29 строк; только 390 — 4; разметка — 22.
...
- 28 браузерных строк не закрыты текущей сессией из-за WinError 5; успешная
  текстовая часть и наличие реализации не заменяют недостающий браузерный PASS.
