...
> состояние репозитория. Текущий live-статус и живая точка входа —
> `docs/RESUME.md`. Этот чек-лист не переписан целиком, чтобы не потерять
> историю приёмки по каждому пункту; новые записи добавлять, старые не
> вычищать.

**Ветка (на момент записи ниже):** `codex/client-approved-copy-only`
