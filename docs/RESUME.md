...
**Версия:** `HANDOFF-RESUME v2.7.0`

**Обновлено:** `2026-09-19`

**Текущий статус:** `final-dev5 LIVE как основная версия + lp.gambarian.com подключён / final-dev3 и final-dev4 закрыты, история ниже архивная`

## Живое состояние прямо сейчас (2026-09-19) — читать первым
...
запуска — `docs/LAUNCH-LP-GAMBARIAN.md`; точки восстановления — `docs/BACKUP-POINTS.md`;
дословные правки владельца этого цикла — `docs/CONTENT-OWNER-EDITS.md` (записи
2026-09-16…2026-09-19).

| Что | Состояние |
|---|---|
| `codex/final-dev5` | рабочая ветка, коммит `7ec7b52`; превью `https://final-dev5.gambarian-landing.pages.dev/` |
...
| Аналитика/трекинг | **не подключены вовсе** — ни GTM, ни GA4, ни пикселя на странице нет; `docs/TRACKING-REQUIREMENTS.md` описывает целевую схему, факт подключения отдельным пунктом не отмечался и остаётся открытым |
...
`lp.gambarian.com` из основной версии (`7ec7b52`).
...
(v0.3.0). Разбор по коду: 58 пунктов в шести группах —
`docs/tasks/2026-09-06-final-dev4-items.md`. Анкета владельцу: 28 вопросов —
`docs/tasks/2026-09-06-final-dev4-questions.md` (v1.2.0, там же ссылка на
интерактивную версию на простом языке). Ответы владельца 2026-09-06 переведены
в «Реестр решений владельца» в spec v0.3.0: 19 из 28 закрыто, 9 открыты и
имеют рекомендации дизайнеров (`docs/DESIGN-RECOMMENDATIONS-2026-09-06.md`) и
замер пустых мест (`docs/SPACING-MEASUREMENTS-2026-09-06.md`). Ключевые
решения: тексты дословно; заголовки без засечек (семейство открыто,
рекомендация — только Onest); фото одинаковые; телефон в нерабочее время везде
→ WhatsApp; alias `final-dev4`, правки в `site/`. Этапы 1–4 порядка реализации
можно начинать; этапы 5–7 ждут ответов или идут «по умолчанию». Замеры шрифтов и фото —
