...
будет отдавать текст без сегодняшних правок.

**Правило деплоя, подтверждённое сегодня:** `workflow_dispatch` виден в Actions
...
`docs/CONTENT-EDIT-PROPOSALS-2026-08-17.md` — 12 предложений по формулировке
(без новых фактов), снятых с живого `final-dev3`. Отдельно там же — найденное
при сверке расхождение: блок `7.6` обещает уточнить «тему обращения», а поле
темы в форме отсутствует. Ждёт ответа владельца по каждой строке.
