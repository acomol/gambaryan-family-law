# Текст на сайте вне клиентского документа

**Версия:** `CONTENT-EXTRA v2.4.2`

**Дата:** `2026-09-07`

**Статус:** `18 OWNER OVERRIDES + SYSTEM-UI VERIFIED / SOURCE PASS / ALL-PREVIEWS BLOCKED / LIVE PENDING`

## Допустимый текст вне 45 номерных блоков

| Категория | Допустимое назначение |
|---|---|
| Identity | Существующие название/логотип бюро и технические metadata/structured data без новых фактов |
| Навигация | Короткие ссылки на существующие секции, управление меню |
| Accessibility | Скрытые подписи и `aria-label`, описывающие действие |
| Форма | Обязательность, placeholders, ошибки, отправка и повтор |
| Action Bar | Рабочее/нерабочее состояние и Preview demo-switch |
| Review | Служебная инструкция и номера `data-copy-id` |

Отдельно разрешены 18 точных `OWNER-APPROVED` блока, перечисленных в
`CONTENT-APPROVED.md` и `CONTENT-SOURCE-MAP.md`:

- 15 owner-блоков правок 2026-09-06, включая `yulia-card-v2`,
  по реестру `CONTENT-OWNER-EDITS.md`;
- Три кубика: `fact-30-v1`, `fact-precedent-v1`, `fact-900-v2`; тексты в CONTENT-OWNER-EDITS.

Это единственные содержательные исключения вне 45 клиентских ID. Остальной
`SYSTEM-UI` не должен содержать юридические факты, обещания результата, новые
специализации или биографию.

## Запрещено новым решением владельца

- поле и подпись `Email`;
- поле и подпись `Тема обращения`/`topic`, даже несмотря на наличие разрешённой
  формулировки `7.19` в клиентском документе;
- «ВПЕРВЫЕ», «СОЗДАН ПРЕЦЕДЕНТ В МЕЖДУНАРОДНОЙ СУДЕБНОЙ ПРАКТИКЕ» и
  «Добились возвращения…»;
- «Или позвоните сразу», «Срочный вопрос? Позвоните напрямую»;
- три прежних proof-текста `final-dev1`;
- сокращённые заголовки, лиды, факты и другие переформулировки утверждённых
  номерных блоков.

Текущий `CLIENT-COPY-VERIFIER v1.1.0` использует
`CLIENT-COPY-CONTRACT v1.4.3 | 2026-09-07` и проверяет
frozen source `docs/sources/client-copy-short-v1.0.0.txt`, принадлежность каждого реально
размещённого смыслового текста к client allowlist или 18 точным owner
overrides, отсутствие полей `Email`/`topic` и известных запрещённых строк.
Контракт охватывает: `26 targets / 24 unique`, `45` client allowlist + `18` owner
blocks, в source/final-dev4 `18` client ID; общий гейт заблокирован сборкой шрифтов. Полный coverage `45/45` не требуется.
Предыдущий PASS `45/45` остаётся историческим; новый кандидат ещё не deployed.

## SYSTEM-UI формы: нерабочее время, этап 4

Решение владельца от 2026-09-07: закрытый вариант нижнего ряда контактов
использует метку «ЗАЯВКА» и подпись «Оставить заявку» дословно.
Обе строки добавлены в `ALLOWED_OUTSIDE_COPY_TEXT`; ссылка ведёт к форме.

Закрытый вариант подсказки: «Если ошибка повторяется, напишите в WhatsApp.».
Разрешённый фрагмент `Если ошибка повторяется, напишите в` добавлен в allowlist
контракта 1.4.1; открытый вариант и номер телефона сохранены дословно.

## Related

- [Карта источников](CONTENT-SOURCE-MAP.md)
- [Правки владельца](CONTENT-OWNER-EDITS.md)
- [Утверждённые 45 блоков](CONTENT-APPROVED.md)
- [Отсутствующие блоки](CONTENT-MISSING.md)
- [Задание](tasks/2026-08-11-client-approved-copy-only.md)
