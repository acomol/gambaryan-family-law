...
| Визуал логотипа | `site/social-preview-logo-v1.0.2-1200x630.png` | копируется штатными сборщиками |
| Публичный image origin | абсолютные URL в metadata block | preview: `https://final-dev.gambarian-landing.pages.dev` |
...
При публикации финала на production заменить image origin на боевой домен и
повысить версию; `noindex` снимается отдельно по инструкции `docs/DEPLOY.md`.
...
- metadata HTML и PNG возвращают `200`, корректные MIME и не блокируются
