# Final Dev 1 — расширенный конверсионный блок Hero на десктопе

**Версия:** `FINAL-DEV1-HERO v1.3.0`

**Дата:** `2026-08-10`

**Документация обновлена:** `2026-08-11` — зависимость Action Bar `2.3.1`;
Hero-контракт не менялся.

**Ветка:** `claude/website-development-kb0fu0`

**Preview:** https://final-dev1.gambarian-landing.pages.dev/

**Контентное дополнение:** `FINAL-DEV1-PRECEDENT-COPY v1.0.0 | 2026-08-11`

В `final-dev1` блок `#precedent` использует утверждённую владельцем редакцию:

- надстрочник: «ВПЕРВЫЕ»;
- заголовок: «СОЗДАН ПРЕЦЕДЕНТ / В МЕЖДУНАРОДНОЙ СУДЕБНОЙ ПРАКТИКЕ»;
- пояснение: «Добились возвращения похищенного ребёнка, / несмотря на
  отсутствие официально / зарегистрированных родительских прав».

Заданные переносы находятся в HTML. На узких экранах браузер может добавить
переносы внутри длинной строки, но не должен создавать overflow или clipping.
Override относится только к производной `final-dev1`: `site/`, production,
остальные Preview, фото и CTA блока не меняются.

## Источник решения

- Референс владельца: [final-dev1-desktop-hero-v1.0.0.png](../design-references/final-dev1-desktop-hero-v1.0.0.png)
- Размер: `1293×724 px`.
- SHA-256: `1E07C0D348AC6C61754D8B05B1FFD1A3F31C8F03B34D85749559707398BC27AB`.
- Уточнение владельца: длинное пояснение не удаляется и располагается ниже
  ряда из трёх иконок.
- Уточнение владельца: на mobile лишнее пустое поле слева от пары убирается
  отдельным кадрированием; desktop-кадр остаётся без изменений.
- Уточнение владельца: подписи преимуществ и длинное пояснение на desktop
  должны читаться как содержательный блок, а не как служебный микротекст.

## Область изменений

Создаётся отдельная производная `build/variants/final-dev1`. Исходный `site/`,
production, существующий `final-dev` и остальные Preview не меняются.

На ширине от `961px` нижняя часть Hero имеет следующий реальный DOM-порядок:

1. основная кнопка «Записаться на консультацию»;
2. золотой разделитель;
3. расширенный блок звонка с круглой иконкой, текстом «Или позвоните сразу»,
   номером `054-549-0623` и подписью «Срочный вопрос? Позвоните напрямую»;
4. три преимущества с декоративными иконками:
   - «Семейное право во всех аспектах»;
   - «Конфиденциальность и защита интересов»;
   - «Индивидуальный подход к каждому делу»;
5. существующий длинный `.hero__note` без изменения текста.

На desktop из шапки удаляется дублирующий телефон справа; четыре ссылки меню
остаются центрированными. Заголовок, вводный текст, фотографии и их кадрирование
на desktop не меняются. Звонок внутри мобильного меню сохраняется.
На ширине до `960px` новый ряд преимуществ скрыт, а звонок и остальной Hero
сохраняют базовую компактную композицию. Опубликованный baseline Action Bar
использует общий контракт `v2.3.0`; Preview release-candidate от `2026-08-11`
обновляет общий addon до `v2.3.1` только для синхронизации верхней подписи с
составом нижней панели. В обеих версиях сохраняются автоматический состав по
рабочему времени Израиля и Preview-переключатель для демонстрации обоих
состояний.

Для мобильных экранов шириной до `860px` и высотой от `600px` вертикальные
интервалы дополнительно сжимаются. Окно фото сохраняет базовую пропорцию `4:3`,
пока она помещается; на широких коротких экранах высота ограничивается
относительно viewport. Фото поднимается, нижняя часть пиджаков обрезается
сильнее, но лица и воздух над головами сохраняются. Обе Hero CTA должны
полностью помещаться без прокрутки минимум на `390×724`, `360×668` и `360×600`.

На mobile до `860px` фото пары увеличивается в `1.15` раза и сдвигается на `7%`
влево внутри прежнего окна. Это уменьшает пустое поле слева, не меняет высоту
Hero и положение CTA; обе головы и волосы остаются внутри кадра. На desktop от
`961px` transform отсутствует. Текст преимуществ имеет размер `12px` до
`1200px` и `13px` от `1201px`, длинное пояснение — соответственно `13px` и
`14px`; контраст и межстрочный интервал повышены.

## Реализация

- Source of truth: вариант `dev1` в `scripts/build-hero-variants.py`.
- Изоляция стилей: `.hero--final-dev1`.
- Маркер версии и даты присутствует в HTML и CSS сборки.
- В Hero остаётся ровно один `.hero__phone`: он служит sentinel для зонной
  модели Action Bar.
- В производной отсутствует `header .nav-call`; основной Hero-телефон и
  `#nav-drawer .nav-drawer__call` остаются рабочими.
- Все SVG преимуществ имеют `aria-hidden="true"`; текст преимуществ остаётся
  доступным в семантическом списке.

## Приёмка

### Статические гейты

- [x] `python scripts/build-hero-variants.py dev1` завершается с кодом `0`.
- [x] `python scripts/verify-client-previews.py` подтверждает точную карту из
  десяти Preview и контракт `final-dev1`.
- [x] `node scripts/verify-lead-hook.mjs` проходит без ошибок.
- [x] `git diff --check` проходит.
- [x] `git diff --exit-code -- site site-addons/action-bar functions` не
  показывает изменений.

### Browser QA

- [x] На `1280×720`, `1440×900` и `1920×1080` видны разделитель,
  расширенный звонок, ровно три преимущества и длинное пояснение ниже них.
- [x] На `1024×768` текст преимуществ не меньше `12px`, пояснение не меньше
  `13px`; на `1280×720` и шире — соответственно `13px` и `14px`. Текст не
  обрезан и не пересекает нижнюю границу Hero.
- [x] DOM- и визуальный порядок совпадают с утверждённым порядком; элементы не
  пересекаются, не обрезаются и не создают горизонтальный overflow.
- [x] На `390×844` новый ряд преимуществ скрыт, фото использует прежний asset и
  окно, но кадр пары плотнее: нет избыточного поля слева, головы не обрезаны;
  Action Bar проходит зоны Hero → чтение → форма.
- [x] На коротких viewport `390×724`, `360×668` и `360×600` обе Hero CTA полностью видны,
  нижняя граница телефонной CTA имеет запас не менее `8px`.
- [x] На широких коротких viewport до `860px` фото не раздвигает Hero ниже CTA;
  на `861px` применяется прежняя desktop/tablet-композиция без скачка overflow.
- [x] На desktop Action Bar не отображается; в консоли нет ошибок приложения.
- [x] Основная кнопка и телефон остаются рабочими ссылками на `#contact` и
  `tel:+972545490623`.
- [x] На desktop телефон справа в шапке отсутствует, а меню остаётся по центру;
  мобильный звонок в шторке сохранён.

### Cloudflare Preview

- [x] `final-dev1.gambarian-landing.pages.dev` возвращает `200` и содержит
  `FINAL-DEV1-HERO v1.3.0 | 2026-08-10`.
- [x] `/lead-contract.js` возвращает JavaScript со схемой `1.1.0`.
- [x] `GET /api/lead` возвращает `405` и `Allow: POST`.
- [x] Production и существующий `final-dev` не содержат маркер `final-dev1`.

## Результат (`v1.3.0`)

- Feature commit: `a207d3f55ce1286c39aefd012c917a72ccb01773`.
- Cloudflare deployment: `4842254e-d861-4c51-951c-5ce462b2ccb1`, status
  `success`, branch `final-dev1`; stable alias и прямой deployment URL отдают
  маркер `v1.3.0`.
- GitHub Actions: [run 31409406158](https://github.com/acomol/gambaryan-family-law/actions/runs/31409406158),
  conclusion `success`.
- Live Browser QA: mobile `360×600`, `390×724`, `390×844`, `860×760` и
  boundary `861×760`; desktop `1024×768`, `1280×720`, `1440×900`. На mobile
  transform включён только до `860px`, обе CTA целиком и horizontal overflow
  `0`; на desktop proof/note имеют `12/13px` и `13/14px` по breakpoint,
  полностью входят в Hero.
- Live Action Bar прошла Hero → чтение → форма: hidden/inert → visible →
  hidden/inert. Fonts loaded, console errors/warnings `0`; production HTML
  SHA-256 сохранился:
  `656cbcd0635952899e79b847d5c262724979d21f548ca66e13fe3a7d2ec13e22`.

## Предыдущий результат (`v1.2.0`)

- Feature commit: `507743eef9903f069a707fafb05baf65b69b88d4`.
- Cloudflare deployment: `c6fd60ad-8b35-4cdb-8081-b94f28c86b28`, status
  `success`, branch `final-dev1`.
- GitHub Actions: [run 31406145891](https://github.com/acomol/gambaryan-family-law/actions/runs/31406145891),
  conclusion `success`.
- Live Browser QA: `360×600`, `360×668`, `390×724`, `390×844`, `720×760`,
  `860×760`, `861×760`, `1280×900`, `1440×900`; обе CTA целиком, horizontal
  overflow `0`, шрифты загружены, console errors/warnings `0`.
- Action Bar на live прошла Hero → чтение → форма: hidden/inert → visible →
  hidden/inert. Production HTML SHA-256 до/после не изменился:
  `656cbcd0635952899e79b847d5c262724979d21f548ca66e13fe3a7d2ec13e22`.

## Предыдущий результат (`v1.1.0`)

- Feature commit: `cc5f6e649ff39aa1f489745df9e5475d04808cb4`.
- Cloudflare deployment: `8a163a38-98c1-4e2c-a836-d610e05e2edb`, status
  `success`, branch `final-dev1`.
- GitHub Actions: [run 31401338940](https://github.com/acomol/gambaryan-family-law/actions/runs/31401338940),
  conclusion `success`.
- Live Browser QA: `1280×720` и `390×844`, console errors/warnings `0`;
  desktop `.nav-call` отсутствует, Hero- и drawer-телефоны сохранены, mobile
  Action Bar прошла Hero → чтение и demo-переключение `2 ↔ 3` действия.

## Related

- [Карта клиентских Preview](../boards/2026-08-06-versions-links.md)
- [Action Bar v2](2026-08-10-action-bar-v2.md)
- [Итоговый QA-чек-лист](../FINAL-QA-CHECKLIST.md)
