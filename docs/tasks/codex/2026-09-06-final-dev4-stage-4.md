...
Убедиться, что влит этап 3 (в <footer> нет tel:). Найти, как этап 1 устроил dev4: константы VERSION/DATE/MARKER/BODY_CLASS в scripts/final_dev4_contract.py; TASK_PATH/BOARD_PATH, где проверяется маркер. ВАЖНО про исходные данные: каталога site-addons/final-dev4/ ПОКА НЕТ — этап 1 намеренно оставил dev4 на адаптере dev3 (build-hero-variants копирует hero-business-hours.js из FINAL_DEV3_ADDON по условию `key in {"dev3", "dev4"}`, и dev3-список required-токенов прогоняется на dev4). Собственный адаптер, HERO_BUSINESS_SCRIPT/HERO_BUSINESS_SCRIPT_TAG, dev4-списки токенов, отдельные ветки dev3/dev4 в build-hero-variants.verify и verify-client-previews и запись в DYNAMIC_UI_PATHS verify-client-copy.py:56–61 создаёт ШАГ 3 ЭТОГО этапа. Проверить перед стартом: test ! -d site-addons/final-dev4 (ожидаемо каталога нет) и grep -n 'key in {"dev3", "dev4"}' scripts/build-hero-variants.py → есть. Имя файла адаптера НЕ менять (иначе каскад правок путей); расширяется содержимое. Проверить латч Action Bar по факту, а не по наличию класса: латч включается по page--final-dev3 (site-addons/action-bar/action-bar.js:50, finalDev3TopOnly), и body dev4 намеренно несёт оба класса — собственного класса dev4 у Action Bar быть НЕ должно, его отсутствие дефектом не считать. Дефект — только фактическая поломка: если на собранном dev4 латч не срабатывает, записать в отчёт и здесь не чинить.

Проверка: awk '/<footer/,/<\/footer>/' site/index.html | grep -c 'tel:' → 0; grep -rn 'FINAL-DEV4-DESIGN v' scripts site-addons docs | одна версия/дата; grep -n 'final-dev4' scripts/verify-client-copy.py → пока пусто; путь адаптера в DYNAMIC_UI_PATHS добавляет шаг 2 этого этапа, на шаге 1 его наличие проверять нельзя
...
Проверка: test -f site-addons/final-dev4/hero-business-hours.js; python -B scripts/build-hero-variants.py → все варианты «проверка пройдена». Порядок обязателен: dev4-списки SCRIPT_REQUIRED_TOKENS с селекторами вне Hero и data-business-closed вводить НЕ здесь, а в шаге 6 вместе с самим поведением адаптера — на этом шаге адаптер ещё байтовая копия dev3, и builder (:446, `any(token not in script …)`) на новых токенах даст FAIL. На шаге 2 список токенов остаётся дословно dev3-овским; cmp build/variants/final-dev4/hero-business-hours.js site-addons/final-dev4/hero-business-hours.js → без вывода; cmp build/variants/final-dev3/hero-business-hours.js site-addons/final-dev3/hero-business-hours.js → без вывода; grep -n 'key in {"dev3", "dev4"}' scripts/build-hero-variants.py → 0; python -B scripts/verify-client-previews.py → PASS; python -B scripts/verify-client-copy.py → PASS

### 3. Разметить в site/index.html, что делать с каждой телефонной ссылкой в состоянии «закрыто».

Файлы: `site/index.html`

Добавить атрибуты (значение текста ссылок не менять): a.nav-call (строка ~54) — data-business-closed="whatsapp"; a.nav-drawer__call (~64) — data-business-closed="whatsapp"; a.hero__call (~93) — data-business-closed="whatsapp" (dev3-адаптер этот атрибут не читает, конфликта нет); a.contact-list__row[href^="tel:"] (~505) — data-business-closed="whatsapp". Это прямой ответ владельца на вопрос №25 («Заменить на WhatsApp, как в Hero», вопрос закрыт) и запись spec:199 «все телефонные ссылки вне Hero заменяются на WhatsApp». Прежняя редакция ставила здесь "hide" по соображению «рядом уже есть ряд WhatsApp» — это решение за владельца, оно снято. Следствие, обязательное к показу в отчёте: в закрытом состоянии в блоке контактов будет два одинаковых ряда WhatsApp; владелец увидит это на Preview и, если сочтёт лишним, отдельным решением вернёт скрытие ряда. Атрибуты ставить после class="…", чтобы регулярка NAV_CALL и replace 'class="hero__call"' в build-hero-variants.py продолжали совпадать. В production/остальных Preview атрибуты инертны (адаптера там нет).

Проверка: grep -c 'data-business-closed="whatsapp"' site/index.html → 4; grep -c 'data-business-closed="hide"' site/index.html → 0; python -B scripts/build-hero-variants.py dev1 dev3 dev4 → без «.nav-call остался» и «пропал звонок»
...
Маркер BUSINESS-HOURS-GATE v1.0.0 | <дата>; аргумент base_url; viewports 390×844 и 1440×900; JSON Lines + итог; exit 0/1. Последовательность на каждом viewport: дождаться .mobile-bar[data-business-state] ∈ {open, closed}; setState(name) — через page.evaluate: document.querySelector('[data-business-demo]').click() (JS-клик работает и при hidden), ждать rAF, сверить атрибут. В open: снять snapshot outerHTML всех [data-business-closed]; для подсказки снимать НЕ контейнер .lead-form__error-contact, а только два span-а [data-business-variant] (их outerHTML). Причина: атрибут hidden на самом контейнере переключает showFormError в site/app.js:411 при показе ошибки доставки, поэтому контейнер до имитации ошибки и после неё отличается по причине, не связанной с адаптером рабочего времени, и сравнение дало бы ложный FAIL. Перевести в closed и проверить: (a) по DOM — [...document.querySelectorAll('a[href^="tel:"]')].filter(a => !a.closest('[hidden]')).length === 0 и по видимости — .filter(a => a.getClientRects().length > 0).length === 0, при открытом меню (.nav-burger click на 390) и при показанной ошибке доставки (page.route('**/api/lead') → 503 JSON, заполнить имя/телефон, submit, дождаться .lead-form__error без hidden и .lead-form__error-contact без hidden); (b) каждый [data-business-closed="whatsapp"]: href/target/rel равны бару [data-business-action="whatsapp"], dataset.action==='whatsapp_click', aria-label и innerText === 'Написать в WhatsApp', svg.innerHTML === svg бара, номер «054-549-0623» в тексте отсутствует; (c) в контактах ряд «Телефон» — это тоже [data-business-closed="whatsapp"], поэтому в closed он проходит проверку (b) наравне с остальными, а видимых a[href^="tel:"] на странице ноль; отдельной проверки на hidden-ряд больше нет (решение владельца №25 — замена, не скрытие); (d) [data-business-variant="closed"] виден и содержит a[href^="https://wa.me/"], [data-business-variant="open"] hidden; (e) на 390 в баре [data-business-action="phone"] hidden (регрессия Action Bar). Перевести в open: outerHTML каждого элемента равен snapshot (для подсказки сравниваются два span-а [data-business-variant], а не контейнер — см. оговорку про app.js:411; если сравнивать контейнер, снимать оба snapshot в одинаковом состоянии формы), ряд «Телефон» виден, hero href === 'tel:+972545490623', открытый вариант подсказки виден. Дополнительно: document.querySelectorAll('a a').length === 0; состояние никогда не 'pending' после ожидания.
...
- `python -B scripts/verify-client-copy.py` — строго ПОСЛЕ сборки: verifier читает build/variants/*/index.html и на чистом checkout вернёт «index.html не найден»
...
- [ ] В closed каждый [data-business-closed="whatsapp"] имеет href/target/rel Action Bar WhatsApp, data-action="whatsapp_click", aria-label и текст «Написать в WhatsApp», иконку WhatsApp; ряд «Телефон» в контактах — тоже [data-business-closed="whatsapp"] и в closed показывает WhatsApp, а не скрыт (решение владельца №25); подсказка ошибки показывает вариант «…напишите в WhatsApp.»
...
- Подтверждение, что защищённые каталоги не изменены ОТ БАЗЫ этапа, а не только относительно индекса: `git diff --stat $(git merge-base main HEAD)..HEAD -- site-addons/final-dev3 site-addons/action-bar` → пусто (голый `git diff` этих путей сравнивает рабочее дерево с индексом и уже закоммиченную правку пропустит); число «&nbsp;—» в build/variants/final-dev4/index.html
...
- Латч finalDev3TopOnly намеренно не расширяется на отдельный класс dev4: body dev4 несёт page--final-dev3, и латч работает через него. Отсутствие расширения дефектом не является; дефект — только фактическая поломка латча на собранном dev4, её фиксировать в отчёте
...
- **Применено** (шаг 3 карточки). Исходное замечание: добавить шаг «Завести собственный адаптер dev4»: git mv/копия site-addons/final-dev3/hero-business-hours.js → site-addons/final-dev4/hero-business-hours.js с маркером FINAL-DEV4-DESIGN; в final_dev4_contract.py задать HERO_BUSINESS_SCRIPT/HERO_BUSINESS_SCRIPT_TAG (то же имя файла), TASK_PATH и списки SCRIPT_REQUIRED/FORBIDDEN_TOKENS dev4; в build-hero-variants.py: FINAL_DEV4_ADDON и копирование для dev4 из него (условие `if key == "dev4"` отдельно от dev3), в verify() отдельный блок dev4 с байт-сравнением против site-addons/final-dev4 и dev4-токенами; в verify-client-previews.py verify_final_dev4 сравнивать с site-addons/final-dev4 и dev4-токенами; в verify-client-copy.py DYNAMIC_UI_PATHS += site-addons/final-dev4/hero-business-hours.js
- **Применено** (в разделе «Версии и маркеры» стоит 1.4.2). Исходное замечание: исправить «PREVIEW-BROWSER-QA-RUNNER 1.4.1» → 1.4.2
- **Применено** (шаг 3: «Действующее решение по латчу: класс page--final-dev3 в body dev4 сохраняется намеренно; Action Bar не бампится»). Исходное замечание: переписать пункт про Action Bar: класс page--final-dev3 в body dev4 сохраняется намеренно (латч finalDev3TopOnly), Action Bar 2.4.0 не бампится; либо перенести этот bump в карточку явно
- **Применено** (в промпте формулировки «созданы этапом 1» больше нет). Исходное замечание: в промпте заменить «созданы этапом 1» на «создать в этом этапе по шагу N»
...
1. site/index.html: добавить data-business-closed="whatsapp" на a.nav-call, a.nav-drawer__call, a.hero__call и на a.contact-list__row[href^="tel:"] — четыре штуки, значение у всех одинаковое (решение владельца №25: заменять, а не скрывать; в закрытом состоянии в контактах окажется два ряда WhatsApp — назвать это в отчёте). Атрибуты после class="…". Подсказку .lead-form__error-contact разбить на <span data-business-variant="open">текущий текст дословно</span><span data-business-variant="closed" hidden>Если ошибка повторяется, напишите в <a href="https://wa.me/972545490623" target="_blank" rel="noopener">WhatsApp</a>.</span>.
...
5. Новый гейт scripts/verify-business-hours.py (Playwright, маркер BUSINESS-HOURS-GATE v1.0.0 | дата) по шагу 9 карточки («Добавить DOM-гейт»; шаг 8 — про контракт копирайта): 390 и 1440; принудительные состояния через [data-business-demo] (JS-клик в page.evaluate); в closed — ноль a[href^="tel:"] вне [hidden] и ноль видимых, включая открытое меню и ошибку доставки (route /api/lead → 503, submit); проверка атрибутов/иконок WhatsApp-замен, ряда «Телефон» в контактах, показывающего WhatsApp, варианта подсказки, панели (phone hidden); в open — outerHTML равен snapshot.
...
Отчёт в PR: хэш и push; список файлов и суть изменений; вывод каждого гейта (JSON Lines verify-business-hours для 390 и 1440); подтверждение, что dev3/action-bar не изменены; «Проверено / Не проверено / Вопросы владельцу» (формулировка подсказки в closed; два одинаковых ряда WhatsApp в контактах как следствие решения №25 — показать скриншотом и спросить, оставлять ли; латч Action Bar для dev4, если он не учтён этапом 1). Деплой делает владелец (Actions → Deploy Previews → ветка этапа → only=final-dev4). До деплоя сними SHA-256 final-dev3 и production (curl -fsS -A gambarian-readback <url> -o rb.tmp && test -s rb.tmp && sha256sum rb.tmp (флаг -f и test -s обязательны: без них недоступный адрес даёт хэш пустоты e3b0c442…b855 и нулевой код); в PowerShell — curl.exe и Get-FileHash). После деплоя: python -B scripts/verify-live-previews.py --only final-dev4; python scripts/verify-business-hours.py https://final-dev4.gambarian-landing.pages.dev/; повтори SHA-256 — должны совпасть (production 656CBCD0…C13E22); допиши proof-блок в PR.
```
