...
**Версия:** `FINAL-DEV4-STAGE-1 v1.0.1`
...
- Деплой final-dev3 и production (локальная пересборка вариантов — не запрет: `build-hero-variants.py` без аргументов пересобирает все варианты из site/, это нужно гейтам; неизменность final-dev3 держится на том, что его алиас не публикуется); запуск wrangler; поле only пустым
...
Импортировать из final_dev4_contract (VERSION/DATE/MARKER_RE/HTML_COMMENT/CSS_COMMENT/BODY_TAG/apply_html_contract/apply_css_contract). def variant_final_dev4(html): html, css = variant_final_dev3(html); return apply_final_dev4_html_contract(html), apply_final_dev4_css_contract(css). VARIANTS["dev4"] = ("final-dev4", "final-dev4: наследник final-dev3 с правками владельцев 2026-09-06", variant_final_dev4) — после dev3. В build(): условие `if key == "dev3":` (копирование hero-business-hours.js из FINAL_DEV3_ADDON + apply_final_dev3_script_contract) → `if key in {"dev3", "dev4"}:`. В verify(): блок `if key in {"dev1", "dev3"}` → `{"dev1", "dev3", "dev4"}`; блок `if key == "dev3":` (начинается :406) → `in {"dev3", "dev4"}`, но ВНУТРИ него проверку body-класса :424–425 сделать зависимой от варианта. Ловушка: `html.count(f'<body class="{FINAL_DEV3_BODY_CLASS}">') != 1` для dev4 всегда даёт 0, потому что тег dev4 двойной — `<body class="page--final-dev3 page--final-dev4">`; расширение блока без этой правки роняет сборку с «final-dev3 должен иметь отдельный scoped body class». Заменить на `expected_body = f'<body class="{FINAL_DEV3_BODY_CLASS}">' if key == "dev3" else FINAL_DEV4_BODY_TAG` и текст ошибки с именем варианта. Это единственная проверка dev3-блока, которую нельзя переносить на dev4 как есть: маркеры dev3 (FINAL_DEV3_MARKER_RE не ловит FINAL-DEV4-DESIGN), комментарии HTML/CSS и адаптер у dev4 на этапе 1 совпадают с dev3 и проходят. Добавить блок `if key == "dev4":` — MARKER_RE.findall(html) == [(VERSION, DATE)] и то же для styles; html.count(HTML_COMMENT) == 1; styles.count(CSS_COMMENT) == 1; html.count(BODY_TAG) == 1. Docstring модуля: добавить строку про dev4 и в пример запуска. Зафиксировать решение: до этапа 4 адаптер dev4 = байт в байт копия site-addons/final-dev3/hero-business-hours.js, dev3-список required-токенов прогоняется и на dev4 (условие `key in {"dev3", "dev4"}`); этап 4 заводит site-addons/final-dev4/ и расцепляет условия dev3/dev4 в build-hero-variants.verify и verify-client-previews. Класс page--final-dev3 в body dev4 сохраняется намеренно (латч Action Bar finalDev3TopOnly), Action Bar 2.4.0 не бампится.
...
Заменить NBSP_EXPECTED/NBSP_EXPECTED_REVIEW_NUMBERED на: NBSP_EXPECTED_DEFAULT = 23 и NBSP_EXPECTED = {"review-numbered": 22, "final-dev4": 23} с комментарием: значение alias = число «&nbsp;—» в релизе, который этот alias отдаёт; для final-dev4 пересчитывать после каждой текстовой правки командой grep -o '&nbsp;—' build/variants/final-dev4/index.html | wc -l; в check_preview: expected = NBSP_EXPECTED.get(branch, NBSP_EXPECTED_DEFAULT). Отдельно добавить проверку идентичности алиаса, которой в скрипте нет вовсе: сейчас check_preview проверяет только HTTP 200, content-type, noindex, счётчик тире, FORBIDDEN_COPY, .nav-links 14px и белое «прецедента» — то есть содержимое final-dev3, выложенное на адрес final-dev4, даёт ложный PASS, а весь смысл этапа 1 в том, что алиас отдаёт именно dev4. Ввести IDENTITY = {"final-dev4": {"html": (f"<!-- FINAL-DEV4-DESIGN v{VERSION}", "page--final-dev4"), "css": (f"/* FINAL-DEV4-DESIGN v{VERSION}",)}} (версию и дату брать из scripts/final_dev4_contract.py импортом, не строкой) и в check_preview для алиасов из IDENTITY требовать все подстроки в живом HTML и в живом styles.css; иначе problems.append(f"{branch}: живая страница не содержит маркер/класс final-dev4 — на алиасе чужая сборка"). Проверить негативом: подставить в проверку тело final-dev3 — должен быть FAIL, а не пустой список. Проверку белого «прецедента» (regex по .fact-card__unit 2.10 в 2 медиаблоках) выполнять только если в живой разметке есть маркер `class="fact-card__unit">прецедента</span>`; иначе пропускать (после перестройки кубиков в этапе 5 появится своя проверка). Docstring строка 4 → `LIVE-PREVIEW-READBACK v1.2.0 | <ДАТА>`; READBACK_VERSION = "1.2.0"; в docstring добавить пример `--only final-dev4`.
...
deploy-previews.yml:3 «Публикация 12 клиентских Preview…», :25 description «Один alias (пусто = все 12)»; deploy-previews.sh:12–13 комментарий «Preview-адресов двенадцать … Двенадцать команд руками — двенадцать шансов…»; docs/DEPLOY.md — четыре места: :85 заголовок «Preview: все двенадцать одной командой», :111–112 «по каждому из 12 адресов», :188 «все 12 адресов сохраняются», :189 «12 вариантов удобнее отдавать подпутями»; docs/RESUME.md:32 «…или пусто для всех 12» и :83–84 «…пусто для всех 12»; docs/CODEX-WORKING-MODEL.md:93 «публикует все двенадцать alias». README.md:33 и CLIENT-PREVIEW-HANDOFF.md описывают исторический пакет из 11 — не трогать. В docs/DEPLOY.md, кроме строки 85, проверить упоминания «11»/«11 адресов» на строках 112, 188, 189 — заменить на 12 либо пометить как историю.

Проверка: grep -n '11' .github/workflows/deploy-previews.yml | grep -v 'v11\|2011' → пусто; grep -n 'одиннадцать' scripts/deploy-previews.sh docs/DEPLOY.md → пусто; grep -nE 'из 11|все 11|11 вариантов' docs/DEPLOY.md → пусто (без пробела после числа: в :111 строка обрывается на «из 11», слово «адресов» перенесено на :112, и шаблон с пробелом её пропускает)
...
client_copy_contract.py:3 → `CLIENT-COPY-CONTRACT v1.4.3 | 2026-09-07` (CONTRACT_VERSION/DATE :11–12 уже такие; не менять). Таблица «Текущие локальные контракты» docs/RESUME.md:119–131: Client Copy contract `1.2.0`; Client Copy verifier `1.1.0`; Review Numbered `2.1.0`; Browser QA runner `1.4.2` (`194/194`); добавить строку `| final-dev4 Design | 1.0.0 | LOCAL PASS |`. docs/CONTENT-APPROVED.md:25 → `v1.2.0` / `v1.1.0`: 26 targets / 24 unique / 45 client + 2 owner. docs/CONTENT-SOURCE-MAP.md:40–42 → `CLIENT-COPY-CONTRACT v1.4.3 | 2026-09-07`, `CLIENT-COPY-VERIFIER v1.1.0 | <ДАТА>`, `26 targets / 24 unique`. boards «Версии контрактов»: Client Copy contract `1.2.0` 2026-08-16; verifier `1.1.0` <ДАТА>; Review numbering `2.1.0` <ДАТА>; Browser QA runner `1.4.2` <ДАТА> `LOCAL PASS 194/194`. docs/FINAL-QA-CHECKLIST.md:55–56, :62, :65 те же значения; версию чек-листа 2.3.1 → 2.3.2, дата <ДАТА>. docs/tasks/2026-08-13-dark-fact-cards.md:80 → `CLIENT-COPY-CONTRACT v1.4.3 | 2026-09-07`, версию документа DARK-FACT-CARDS 1.0.1 → 1.0.2 с датой. Файлы docs/reviews/*, CLIENT-PREVIEW-HANDOFF.md, ERRORS.md — история, не трогать.
...
- scripts/client_copy_contract.py:3 docstring → CLIENT-COPY-CONTRACT v1.4.3 | 2026-09-07 (синхронизация без bump; CONTRACT_VERSION остаётся 1.2.0) → RESUME:121, CONTENT-APPROVED:25, CONTENT-SOURCE-MAP:40, boards, FINAL-QA-CHECKLIST:55, dark-fact-cards.md:80
...
- **Применено.** Исходное замечание: «Исправить номер строки docstring runner (6, не 15) и добавить DEPLOY.md:112/188/189 в шаг 10 либо явно пометить их историей». Маркер runner-а в карточке указан как docstring :2 — сверено по scripts/qa-browser-matrix.py:2 (значения 15 и 6 оба неверны); счётчики DEPLOY.md :111–112, :188, :189 внесены в шаг 10 вместе с проверкой grep
- **Применено** (шаг 2: TASK_PATH = docs/tasks/codex/README.md, туда же пишется маркер, шаг 5 его проверяет). Исходное замечание: в final_dev4_contract.py задать TASK_PATH (как у final_dev3_contract) или явно написать «TASK_PATH нет» — этапы 4, 7, 8 ссылаются на «документ TASK_PATH из final_dev4_contract.py», которого карточка этапа 1 не определяет
- **Применено** (карточка этапа 4, шаг 3 расцепляет `key in {"dev3", "dev4"}` обратно на `key == "dev3"` и заводит dev4-токены). Исходное замечание: явно зафиксировать в шаге 4/5, что dev3-список FINAL_DEV3_SCRIPT_REQUIRED_TOKENS (build-hero-variants.py:437, verify-client-previews.py:81) будет прогоняться и на адаптере dev4 через `key in {"dev3","dev4"}` — этап 4 обязан это расцепить (см. cross_stage)
- **Подтверждение критика** (не правка). Все проверки, файлы и номера строк подтверждены по дереву a5de24d: verify-client-copy.py:38, verify-client-previews.py:61–76/:306/:375, qa-browser-matrix.py:62/:108–124/:809/:1033/:1115, deploy-previews.yml:3/:25, build-hero-variants.py:305/:315/:359/:406, verify-live-previews.py:4/:30/:117–124, client_copy_contract.py:3/:11–12, styles.css:657/:663/:1572/:1596, app.js:602/:615, review_numbered_contract.py:6–9, build-review-numbered.py:177–200, RESUME:32/:83–84/:119–131, CONTENT-APPROVED:25/:149/:152, SOURCE-MAP:40–42/:75/:78, FINAL-QA:55–56/:62/:65/:683, dark-fact-cards.md:80, CODEX-WORKING-MODEL:93; 12 текущих тестов → 13; 26 targets/24 unique при 12 alias

Карточка точная и самодостаточная; единственные погрешности — номер строки docstring и неполный список «11» в DEPLOY.md. Ключевой скрытый долг: контракт dev4 без TASK_PATH и общий с dev3 набор required-токенов адаптера, на которые опирается этап 4.
