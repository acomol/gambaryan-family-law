...
Проверка: ls docs/design-references | grep -c 'facts-dark-.*v2.0.0.png' → 2; python -c "from PIL import Image  # Pillow: в requirements-build.txt его закрепляет этап 7, к этому шагу ставить вручную — python -m pip install Pillow;print(Image.open('docs/design-references/facts-dark-1440-v2.0.0.png').size[0], Image.open('docs/design-references/facts-dark-390-v2.0.0.png').size[0])" → 1440-ширина контейнера и 390 (ширина секции) — снимки не пустые
