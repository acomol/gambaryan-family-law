...
Файлы (только чтение, шаг ничего не меняет): `docs/tasks/codex/`, `docs/tasks/2026-09-06-final-dev4-spec.md`, `scripts/client-preview-map.json`, `scripts/final_dev4_contract.py`
...
python -m http.server 8098 из корня (отдельный терминал). python scripts/verify-address-links.py http://127.0.0.1:8098/build/variants/final-dev4/; python scripts/verify-business-hours.py http://127.0.0.1:8098/build/variants/final-dev4/; python scripts/measure-fonts.py http://127.0.0.1:8098/build/variants/final-dev4/ (с --expect-heading <семейство>, если этап 7 выполнил Часть B); python scripts/measure-head-top.py --url http://127.0.0.1:8098/build/variants/final-dev4/ --out /tmp/head-s8; python scripts/measure-section-gaps.py http://127.0.0.1:8098/build/variants/final-dev4/ --expect-desktop <GAP_DESKTOP из отчёта этапа 7> --expect-mobile <GAP_MOBILE из отчёта этапа 7> (значения по умолчанию 80 и 48; зашивать их нельзя — при другом решении владельца верный результат этапа 7 получит FAIL) --allow hero,facts,footer; python scripts/qa-browser-matrix.py http://127.0.0.1:8098/build/variants/final-dev4/ --target-name final-dev4; python scripts/qa-browser-matrix.py http://127.0.0.1:8098/ --all-previews (итог ячеек — значение после этапов 6–7, записать); git diff --check. Любой FAIL — стоп: дефект в отчёт, этап не продолжается до фикса отдельным PR.
...
Файлы: `docs/FINAL-QA-CHECKLIST.md`, `docs/boards/2026-08-06-versions-links.md`, `docs/CLIENT-PREVIEW-HANDOFF.md`, `CHANGELOG.md`, `docs/CODEX-WORKING-MODEL.md`

FINAL-QA-CHECKLIST: версия минорный bump + дата; §2 таблица — все текущие версии (сверка с шагом 4); §13 новая подсекция «final-dev4 v<версия> — LIVE PASS»: run id, deployment URL, SHA-256 трёх адресов до/после, список live-гейтов; §17 — строка «карта 2.5.0 содержит final-dev4»; §16 — статусы, закрытые циклом (C2, C6 из этапа 7, остальные не трогать). boards/versions-links: версия документа 2.7.1 → 2.8.0, заголовок «Двенадцать…», строка 12 final-dev4 → «LIVE PASS, релиз <хэш>», таблица «Версии контрактов» — актуальные значения и строка final-dev4 LIVE PASS, Marker final-dev4 актуальный; «Карта пересборки» — строка dev4 (builder hero-variants + site-addons/final-dev4). CLIENT-PREVIEW-HANDOFF: 2.4.1 → 2.5.0, статус «final-dev3 LIVE (эталон) + final-dev4 LIVE + VERIFIED / остальные 10 LIVE PENDING / PRODUCTION UNCHANGED»; строка URL https://final-dev4.gambarian-landing.pages.dev/ с кратким отличием (правки владельцев 2026-09-06: тексты, адрес-ссылка, подвал, нерабочее время, кубики, услуги, фото, отступы, шрифты — по факту); «Что должен проверить клиент» — пункты по открытым вопросам (кадр фото, шрифт заголовков, отступы); чек-лист «Перед отправкой» — новые отметки с run id. CHANGELOG.md [Unreleased]: подраздел «### Added» — «final-dev4 Preview (alias final-dev4): owner revisions 2026-09-06 — stage 1–7 (PR #…)» одной строкой на этап; «### Changed» — контракты с версиями. CODEX-WORKING-MODEL 1.0.0 → 1.1.0: раздел «Журнал этапов цикла final-dev4» — таблица этап / ветка / PR / merge-хэш / деплой run / статус, и одна строка уроков (что в модели работало, что нет — по фактам PR-ревью). spec НЕ править. Для архитектора собрать в отчёте PR раздел «Данные для приёмки spec»: предлагаемый статус в шапке («РЕАЛИЗОВАН, final-dev4 LIVE <ДАТА>; открытые вопросы — см. status-документ») и построчно чек-боксы раздела «Приёмка» — какие закрываются и каким proof, какие остаются открытыми и почему; отдельной строкой — отклонение от подготовительного шага 9 spec: документ docs/tasks/<дата>-fact-cards-hierarchy.md не создавался, контракт кубиков переиздан как DARK-FACT-CARDS v2.0.0 в docs/tasks/2026-08-13-dark-fact-cards.md (этап 5), а docs/tasks/<дата>-mobile-services.md создан этапом 6.

Проверка: grep -n 'final-dev4' docs/FINAL-QA-CHECKLIST.md docs/boards/2026-08-06-versions-links.md docs/CLIENT-PREVIEW-HANDOFF.md CHANGELOG.md docs/CODEX-WORKING-MODEL.md → в каждом ≥1; git diff --name-only → docs/tasks/2026-09-06-final-dev4-spec.md отсутствует (spec ведёт архитектор); git diff --check → пусто
...
- `python scripts/measure-section-gaps.py http://127.0.0.1:8098/build/variants/final-dev4/ --expect-desktop <GAP_DESKTOP из отчёта этапа 7> --expect-mobile <GAP_MOBILE из отчёта этапа 7> (значения по умолчанию 80 и 48; зашивать их нельзя — при другом решении владельца верный результат этапа 7 получит FAIL) --allow hero,facts,footer`
...
- `git diff --check`
- `до деплоя: curl -fsS -A gambarian-readback https://final-dev3.gambarian-landing.pages.dev/ -o rb.tmp && test -s rb.tmp && sha256sum rb.tmp; curl -fsS -A gambarian-readback https://gambarian-landing.pages.dev/ -o rb.tmp && test -s rb.tmp && sha256sum rb.tmp (656CBCD0…C13E22); python -B scripts/verify-live-previews.py --only final-dev4`
...
- `после деплоя: curl -fsS -A gambarian-readback https://final-dev4.gambarian-landing.pages.dev/ -o rb.tmp && test -s rb.tmp && sha256sum rb.tmp = sha256sum build/variants/final-dev4/index.html; grep-маркеры (FINAL-DEV4-DESIGN, page--final-dev4, map_click=3, «Связь»=0, card-v3 — по отчёту этапа 7 (4 при перекадрировании обеих карточек), svc-eyebrow=0)`
...
- docs/tasks/2026-09-06-final-dev4-spec.md: FINAL-DEV4-SPEC patch bump (статус, отметки приёмки) — ставит архитектор по отчёту этапа, Codex файл не трогает
...
- [ ] RESUME 2.7.0, FINAL-QA-CHECKLIST (версия, §2, §13, §17), boards 2.8.0 (12 alias, версии), CLIENT-PREVIEW-HANDOFF 2.5.0 (URL final-dev4), CHANGELOG [Unreleased], CODEX-WORKING-MODEL 1.1.0 (журнал этапов), раздел «Данные для приёмки spec» в отчёте PR (отметки ставит архитектор) — версии в документах совпадают с кодом
...
- **Применено** (шаги 4 и 8 и промпт ссылаются на отчёт этапа 7). Исходное замечание: ожидание «card-v3 → 4» и статусы строк 2/15/…/69 привязать к фактическому исходу этапа 7 (Часть B и пропорция фото могут быть отложены) — сформулировать как «по отчёту этапа 7», не как константу
- **Применено.** Шаг 10 больше не правит spec — Codex готовит раздел «Данные для приёмки spec» в отчёте, отметки ставит архитектор (исходное замечание: «Шаг 10 правит spec силами Codex — согласовать с ролевой моделью»)
- **Применено** (шаг 6: сверять SHA из лога прогона с git rev-parse origin/codex/final-dev4-s8-release). Исходное замечание: шаг 6 — предупредить, что до коммита ветка codex/final-dev4-s8-release = main, и run id надо сверять с фактическим SHA ветки в логе workflow
...
3. python -m http.server 8098 из корня; по http://127.0.0.1:8098/build/variants/final-dev4/: verify-address-links, verify-business-hours, measure-fonts (--expect-heading <семейство>, если этап 7 выполнил Часть B), measure-head-top --out /tmp/head-s8, measure-section-gaps --expect-desktop <GAP_DESKTOP из отчёта этапа 7> --expect-mobile <GAP_MOBILE из отчёта этапа 7> (значения по умолчанию 80 и 48; зашивать их нельзя — при другом решении владельца верный результат этапа 7 получит FAIL) --allow hero,facts,footer, qa-browser-matrix --target-name final-dev4; qa-browser-matrix http://127.0.0.1:8098/ --all-previews; git diff --check.
...
5. До деплоя: curl -fsS -A gambarian-readback https://final-dev3.gambarian-landing.pages.dev/ -o rb.tmp && test -s rb.tmp && sha256sum rb.tmp; то же для https://gambarian-landing.pages.dev/ (ожидается 656CBCD0…C13E22, 52 872 байта) и https://final-dev4.gambarian-landing.pages.dev/; python -B scripts/verify-live-previews.py --only final-dev4. В PowerShell — curl.exe и Get-FileHash.
...
8. curl final-dev4 → sha256sum равен sha256sum build/variants/final-dev4/index.html; grep -c: FINAL-DEV4-DESIGN v<версия> → 1, page--final-dev4 → 1, data-action="map_click" → 3, site-footer__label">Связь → 0, card-v3 → по отчёту этапа 7 (4 при перекадрировании обеих карточек), svc-eyebrow → 0.
...
12. CHANGELOG.md — запись в [Unreleased]; docs/CODEX-WORKING-MODEL.md → 1.1.0 (журнал этапов: этап / ветка / PR / хэш / run / статус); spec не трогать: статус в шапке и отметки «Приёмки» ставит архитектор — подготовь для него раздел «Данные для приёмки spec» в отчёте PR.
