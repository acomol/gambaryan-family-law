...
**Версия:** `CONTACT-LINKS-SPEC v1.2.0` · **Обновлено:** `2026-09-23`
...
| Что | Значение |
| --- | --- |
| Телефон, E.164 (звонки, `tel:`) | `+972545490623` — Александр Гамбарян, не менялся |
| WhatsApp-формат (`wa.me`) | `972587803188` (без `+`, нулей и дефисов) — стажёр; решение владельца 2026-09-23: WhatsApp принимает не тот номер, на который идут звонки |
...
```html
<a class="map-link" data-action="map_click" href="https://www.google.com/maps/search/?api=1&amp;query=%D7%A7%D7%A8%D7%9C%D7%99%D7%91%D7%9A+10%2C+%D7%AA%D7%9C+%D7%90%D7%91%D7%99%D7%91"
   target="_blank" rel="noopener"
   aria-label="Открыть адрес в Google Maps: Тель-Авив, Карлибах, 10">
  Прием&nbsp;— Тель-Авив / онлайн<br>
  <span class="map-link__address">Карлибах,&nbsp;10</span>
</a>
```

Решение №8 от 2026-09-06: кликабельна вся строка с иконкой, вложенных `<a>` нет.
Во всех трёх местах адрес подчёркнут; в подвале адрес идёт перед строкой «Прием…».
...
> **Действующее исключение Gambarian (`2026-08-11`, номер обновлён `2026-09-23`).**
> Для этого лендинга предзаполненный текст не разрешён действующим
> allowlist-контрактом. Все WhatsApp-ссылки текущего кандидата используют ровно
> `https://wa.me/972587803188`, без query `?text=`. Общий синтаксис выше остаётся
