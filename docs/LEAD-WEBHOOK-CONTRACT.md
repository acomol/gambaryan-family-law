...
**Статус:** `LIVE`. `ALBATO_WEBHOOK_URL` установлен владельцем 2026-09-23;
сценарий Albato `GAMB_ADV` (bundle `389465`) — живой, а не catch-заготовка:
`Webhook → Google Sheets «Входящие» (create/update по submission_id; on
error: Continue) → Gmail «Send email» → Answer`. Контрольная заявка
`b4f5e872` (24.09, 08:30 IDT) дошла до всех 5 адресатов офисным письмом за
2 секунды — источник: `docs/STATE-2026-09-24.md`. Downstream-dedupe
по `submission_id` и утверждённый privacy notice — статус **не проверено**
этой сессией, ниже по тексту остаются как открытые пункты приёмки (см.
«Секреты и приёмка»).
...
### Дополнительные поля только для Albato: `email_subject`, `email_html`

С `2026-09-24` (владелец, живой прод) JSON, уходящий в Albato, несёт ещё два
поля — **только для шага Gmail внутри сценария `GAMB_ADV`**, к схеме
`2.4.0`/`site/lead-contract.js` (браузер↔`/api/lead`) отношения не имеют и
поэтому `schema_version` не поднимают:

| Поле | Источник | Содержимое |
| --- | --- | --- |
| `email_subject` | `shared/lead-email.js`, `renderNewLeadEmail()` | `"Новая заявка с сайта — <имя>"`; control-символы (`\x00`–`\x1F`, `\x7F`) вырезаны (`stripSubjectControlChars`) — защита от email header injection (RFC 5322 §2.2) |
| `email_html` | `shared/lead-email.js`, `renderNewLeadEmail()` | Полное HTML-письмо офисного уведомления (брендинг «Гамбарян & Партнёры», контактный блок, кнопки «Позвонить»/«WhatsApp», ссылка на таблицу); ≈6.7 КБ, лимит поля Albato 20 КБ соблюдён |

Рендерятся **из RAW значений** (`fields`, тех же, что идут в остальной
payload) — **до** применения `sheetSafePayload()`. Поэтому `email_html`/
`email_subject` никогда не содержат экранирующий апостроф, который
Albato-Sheets-шаг требует для `name`/`phone`/`email`/`landing_path`/
`referrer_host`/UTM/click-id (см. «Защита таблицы» выше) — апостроф остаётся
правилом только для полей, которые Albato кладёт в Google Sheets; человек,
читающий письмо, апострофа не видит. Каждое значение экранировано через
`escapeHtml()` отдельно от sheet-safe экранирования.

Модуль `shared/lead-email.js` — чистые функции без доступа к `fetch`/KV/D1/R2,
импортируется идентично `forwardToAlbato` в `functions/api/lead.js` и в
`cron-worker/src/index.js` (ретрай из cron рендерит письмо заново из
`payload_json`, не хранит готовый HTML). Это письмо уходит **в офис** (5
адресов: `cityr.ta@gmail.com`, `justicetelaviv@gmail.com`,
`gambarian@gmail.com`, `alex@adfix.co.il`, `nat.shurygin@gmail.com`) через
Gmail-подключение Albato (`4774022@gmail.com`, From `4774020@gmail.com`) —
это отдельная сущность от письма-подтверждения самому заявителю, транспорта
для которого в этом коде по-прежнему нет (см. «Локальная проверка и
границы» ниже).
...
Почтового транспорта в `functions/api/lead.js` (или в `cron-worker`) нет:
только fetch к `ALBATO_WEBHOOK_URL`, а собственно письмо шлёт Gmail-шаг
внутри Albato по полям `email_subject`/`email_html` (раздел выше). Это
письмо — **офисное уведомление**, не подтверждение заявителю: адресаты —
5 адресов офиса, не e-mail из формы. Письмо-подтверждение самому заявителю
(с его собственным адресом в `To`) по-прежнему требует отдельного
подключения транспорта и утверждённого адреса офиса для reply-to — в этой
задаче не подключается.
