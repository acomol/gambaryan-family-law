...
| Поле | Тип | Источник |
| --- | --- | --- |
| `schema_version` | string | сервер, `2.2.0` |
...
- первый submit показывает `.lead-form__confirm` без сетевого запроса; поля скрыты,
  израильский телефон показан группами (`+972 50-000-0000`), строка «Связь» берётся
  дословно из подписи выбранной радиокнопки.
...
- успех по каналу: «Мы свяжемся с вами по телефону <displayPhone>»,
  «Мы напишем вам в WhatsApp: <displayPhone>», «Мы ответим на e-mail: <email>».
  После точки во всех случаях: «Вы указали: <displayPhone>, <email>».
- после принятия браузер сохраняет ID ответа и снимок имени/телефона/e-mail/канала.
...
- `lead-contract.js?v=2.2.0` и `app.js?v=2.2.0` подключаются согласованно;
  `version` и `schemaVersion` общей карты равны `2.2.0`. Несовпадение `version`
...
1. Albato `Catch a Webhook` получает полный синтетический payload `2.2.0`:
   присутствуют `name`, `phone`, `email`, `channel`, `corrects_submission_id`; `topic` отсутствует.
