...
**Версия правил:** `PROJECT-AGENT-RULES v1.4.0`
...
- Сохранять существующие HTML/CSS/JavaScript и Python conventions; не
  рефакторить соседний код без требования задачи и не добавлять поведение,
  которого не просили: каждая строка диффа должна указывать на конкретную
  фразу владельца или пункт задания.
- Работающий блок меняется диффом к `site/`. Прототип с нуля — отдельный
  артефакт для показа, а не способ правки: заново написанный блок теряет
  обкатанные мелочи и приходится сверять их по одной.
