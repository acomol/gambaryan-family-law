...
(function () {
  ...
  function enqueueOutbox(data) {
    if (data.lf_hp) return; // ловушка — не лид, ретраить нечего
    ...
    submitLead(entry.data).then(
      function (response) {
        ...
        fireLeadOnce(submissionId, {
          ...
          seconds_to_lead: Math.max(0, Math.round((Date.now() - entry.created_at) / 1000)),
        });
      },
      function (error) {
        if (error.status === 422) {
  ...
  function flushOutbox(useBeacon) {
    ...
    entries.forEach(function (entry) {
      if (now - entry.created_at > OUTBOX_TTL_MS) {
        sendDeadLetterBeacon("ttl", entry.attempts || 0);
        changed = true;
        return;
      }
      ...
      alive.push(entry);
      changed = true;
      deliverOutboxEntry(entry, useBeacon);
    });
  ...
  if (form && success) {
    ...
    form.addEventListener("submit", function (event) {
      ...
      hideFormError();
      setSubmitting(true);
      enqueueOutbox(data); // write-ahead: до сети, чтобы сбой fetch не терял заявку
        ...
        function (response) {
          ...
          if (!data.lf_hp) {
            if (editingContacts) {
              ...
              fireLeadOnce(acceptedSubmissionId, {
                submission_id: acceptedSubmissionId,
