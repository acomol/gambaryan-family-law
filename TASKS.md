...
- [x] **Оформленное письмо о заявке (2026-09-24 02:12 IDT):** сервер рендерит `email_html`/`email_subject` (`shared/lead-email.js`, деплой 71fe20bc), Albato Gmail = HTML только из этих полей. Контроль 044c298b: письмо через 2 с всем 5; tel:+972501234567, wa.me/972501234567, без апострофа. Паузы Albato при настройке 24–73 с ×6 — заявок в окнах не было (D1)
- [x] Тестовые заявки G-0001..G-0006 → «Дубль / спам»; «Сегодня»: 0 новых (напоминаний 09:30 не будет)
- [x] **Письмо v2 по замечаниям владельца (2026-09-24 08:30 IDT):** компактная шапка → «Новая заявка» + время → блок контактов (подпись над значением) → 2 кнопки на всю ширину 52–56 px, промежуток 16–20 px → служебный блок → «Открыть таблицу заявок» (нейтральная); плашка «Тестовая заявка — не обрабатывать» при utm_source*test. Контраст ≥4.5:1 (таблица в docs/LEAD-PIPELINE.md §16). Деплой 1e1523a0; контроль b4f5e872 → письмо через 2 с. Приёмка — владелец в Gmail на телефоне (светлый/тёмный)
