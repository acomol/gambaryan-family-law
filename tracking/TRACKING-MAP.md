...
| Сайт: HTML деплоя (sha256, короткая форма) | **`b30026ff27b0d6bc`**, не менялся с 2026-09-23 | `docs/LAUNCH-LP-GAMBARIAN.md:39` (кросс-ссылка из другого документа этого репо, самостоятельно не пересчитывался этим прогоном — см. «Не проверено» ниже) |
| Код: git-коммиты, на которых реально гонялся автотест | `4451006` (2026-09-23, первая сверка) и `94687e0` (2026-09-24, эта сверка) — **оба PASS**, `site/app.js` идентичен в обоих | `git log`, `verify-tracking.mjs` |
...
| 42 | «Отправить ещё одну заявку» → 202 (новый submission_id) | `generate_lead` {form_id, submission_id, seconds_to_lead} | T24→G38+A53 | **план: да** [unverified] | [1][2] |
...
- **`docs/LAUNCH-LP-GAMBARIAN.md:39`** сам отмечает как не проверенное на 24.09: входит ли
  noindex-слой в текущий живой деплой. Sha `b30026ff27b0d6bc` в шапке этой карты взят из того
  документа как есть — самостоятельно пересчитыван (curl + sha256) в рамках ЭТОЙ карты не был.
- GA4 Admin: реально ли `generate_lead` помечен как «ключевое событие» звёздочкой — ни один из
  трёх источников это не проверял ([unverified], см. строки 37/42 выше).
